...
static int32_t vdi_dispatch(void *opaque, uint16_t opcode,
                            ...
                            int16_t *ptsout, uint16_t ptsout_capacity, uint16_t *ptsout_count)
{
    ...
    intout[0]=42; ptsout[0]=320; ptsout[1]=200; *intout_count=1u; *ptsout_count=2u; return 0;
}
...
int main(void)
{
    struct amtari_context ctx={0}; struct vdi_fixture f={0}; int16_t intin[2]={7,9},ptsin[2]={10,20},intout[4]={0},ptsout[4]={0}; uint16_t ni=0u,np=0u; uint8_t m[512]={0};
    assert(amtari_vdi_bind(&ctx,vdi_dispatch,&f)==AMTARI_EINVAL); assert(amtari_init(&ctx)==0);
    assert(amtari_vdi_dispatch(&ctx,1u,intin,2u,ptsin,2u,intout,4u,&ni,ptsout,4u,&np)==AMTARI_ENOSYS);
    assert(amtari_vdi_bind(&ctx,vdi_dispatch,&f)==0); assert(amtari_vdi_dispatch(&ctx,100u,intin,2u,ptsin,2u,intout,4u,&ni,ptsout,4u,&np)==0);
    assert(f.calls==1 && f.opcode==100u && ni==1u && np==2u && intout[0]==42 && ptsout[0]==320 && ptsout[1]==200);

    /* Real TOS VDI ABI: D0.W=$73, D1 -> five-longword VDIPB. Output counts are returned in contrl[4]/contrl[2]. */
    ...
    put16(m,0x40u,100u); put16(m,0x42u,1u); put16(m,0x44u,0u); put16(m,0x46u,2u); put16(m,0x48u,0u);
    ...
    assert(get16(m,0x44u)==1u && get16(m,0x48u)==1u);
    ...
    assert(amtari_vdi_bind(&ctx,0,0)==0); assert(amtari_vdi_dispatch(&ctx,100u,intin,2u,ptsin,2u,intout,4u,&ni,ptsout,4u,&np)==AMTARI_ENOSYS);
    return 0;
}
