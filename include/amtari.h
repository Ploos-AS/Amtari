...
#include <stddef.h>
#include <stdint.h>
#define AMTARI_VERSION "0.3.4-m3"
...
typedef int (*amtari_vdi_line_fn)(void *opaque,int16_t x1,int16_t y1,int16_t x2,int16_t y2);
...
struct amtari_vdi_io { amtari_vdi_dispatch_fn dispatch; amtari_vdi_line_fn line; void *opaque; void *line_opaque; uint16_t width,height,colors,next_handle; };
...
int amtari_guest_memory_bind(struct amtari_context*,uint8_t*,size_t); int amtari_guest_range_valid(const struct amtari_context*,uint32_t,size_t); int amtari_guest_read16(const struct amtari_context*,uint32_t,uint16_t*); int amtari_guest_read32(const struct amtari_context*,uint32_t,uint32_t*); int amtari_guest_write16(struct amtari_context*,uint32_t,uint16_t);
