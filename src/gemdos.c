...
static int32_t gemdos_dcreate(struct amtari_context *ctx)
{
    ...
    return amtari_gemdos_error_from_host(ctx->fs.mkdir(ctx->fs.opaque, path));
}
...
static int32_t gemdos_ddelete(struct amtari_context *ctx)
{
    ...
    return amtari_gemdos_error_from_host(ctx->fs.rmdir(ctx->fs.opaque, path));
}
...
static int32_t gemdos_dsetpath(struct amtari_context *ctx)
{
    ...
    if (translated[0] >= 'A' && translated[0] <= 'Z' && translated[1] == ':') {
        drive = (uint8_t)(translated[0] - 'A');
    }
    relative = translated + 3;
    if (strlen(relative) >= AMTARI_PATH_MAX) {
        return AMTARI_EINVAL;
    }
...
static int32_t gemdos_dgetpath(struct amtari_context *ctx)
{
    ...
    if (read_arg32(ctx, 0, &buffer) != 0 || read_arg16(ctx, 4, &drive_arg) != 0) {
        return AMTARI_EFAULT;
    }
    if (drive_arg == 0u) {
        drive = ctx->current_drive;
    } else if (drive_arg <= 26u) {
        drive = (uint8_t)(drive_arg - 1u);
    } else {
        return AMTARI_ENOENT;
    }
    if ((ctx->drive_mask & (1u << drive)) == 0u) {
        return AMTARI_ENOENT;
    }

    length = strlen(ctx->cwd[drive]) + 1u;
    if (!amtari_guest_range_valid(ctx, buffer, length)) {
        return AMTARI_EFAULT;
    }
...
static int32_t gemdos_fcreate(struct amtari_context *ctx)
{
    ...
    return amtari_gemdos_error_from_host(ctx->fs.create(ctx->fs.opaque, path, attr));
}
...
static int32_t gemdos_fopen(struct amtari_context *ctx)
{
    ...
    return amtari_gemdos_error_from_host(ctx->fs.open(ctx->fs.opaque, path, mode));
}
...
static int32_t gemdos_fread(struct amtari_context *ctx)
{
    ...
    return amtari_gemdos_error_from_host(ctx->fs.read(ctx->fs.opaque, (int16_t)handle,
                                                      &ctx->memory.data[buffer], count));
}
...
static int32_t gemdos_fwrite(struct amtari_context *ctx)
{
    ...
    return amtari_gemdos_error_from_host(ctx->fs.write(ctx->fs.opaque, (int16_t)handle,
                                                       &ctx->memory.data[buffer], count));
}
...
static int32_t gemdos_fdelete(struct amtari_context *ctx)
{
    ...
    return amtari_gemdos_error_from_host(ctx->fs.unlink(ctx->fs.opaque, path));
}
...
static int32_t gemdos_fseek(struct amtari_context *ctx)
{
    ...
    if (read_arg32(ctx, 0, &raw_offset) != 0 || read_arg16(ctx, 4, &handle) != 0 ||
        read_arg16(ctx, 6, &mode) != 0) {
        return AMTARI_EFAULT;
    }
    if (mode > 2u) {
        return AMTARI_EINVAL;
    }
    return amtari_gemdos_error_from_host(ctx->fs.seek(ctx->fs.opaque, (int16_t)handle,
                                                      (int32_t)raw_offset, mode));
}
