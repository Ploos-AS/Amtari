...
#define AMTARI_VERSION "0.3.3-m3"
...
struct amtari_console_io { amtari_console_getc_fn getc; amtari_console_putc_fn putc; amtari_console_status_fn input_ready; amtari_console_status_fn output_ready; void *opaque; };
struct amtari_clock_io { amtari_clock_get_fn get; amtari_clock_set_fn set; void *opaque; };
struct amtari_vdi_io { amtari_vdi_dispatch_fn dispatch; void *opaque; uint16_t width; uint16_t height; uint16_t colors; uint16_t next_handle; };
struct amtari_fs_io { amtari_fs_open_fn open; amtari_fs_create_fn create; amtari_fs_close_fn close; amtari_fs_read_fn read; amtari_fs_write_fn write; amtari_fs_seek_fn seek; amtari_fs_delete_fn unlink; amtari_fs_mkdir_fn mkdir; amtari_fs_rmdir_fn rmdir; void *opaque; };
...
struct amtari_mem_block { uint32_t address; uint32_t size; uint32_t owner_basepage; uint8_t valid; uint8_t in_use; };
...
struct amtari_context {
    ...
    uint32_t drive_mask; uint8_t current_drive; char cwd[26][AMTARI_PATH_MAX]; uint32_t next_load_address; uint32_t current_basepage;
    struct amtari_mem_block mem_blocks[AMTARI_MEM_BLOCK_MAX]; uint32_t heap_top; uint32_t random_seed; uint8_t process_depth; int initialized;
};
...
int amtari_clock_bind_rw(struct amtari_context *ctx, amtari_clock_get_fn get_fn, amtari_clock_set_fn set_fn, void *opaque);
...
int amtari_console_status_bind(struct amtari_context *ctx, amtari_console_status_fn input_ready_fn, amtari_console_status_fn output_ready_fn);
int amtari_fs_bind(struct amtari_context *ctx, amtari_fs_open_fn open_fn, amtari_fs_create_fn create_fn, amtari_fs_close_fn close_fn, amtari_fs_read_fn read_fn, amtari_fs_write_fn write_fn, amtari_fs_seek_fn seek_fn, amtari_fs_delete_fn delete_fn, amtari_fs_mkdir_fn mkdir_fn, amtari_fs_rmdir_fn rmdir_fn, void *opaque);
...
int32_t amtari_prg_load_reserved(struct amtari_context *ctx, const uint8_t *image, size_t image_size, uint32_t basepage, const uint8_t *cmdline, uint32_t tail_reserve);
