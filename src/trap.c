...
{
    uint16_t dev, value;
    if (bios_read_arg16(ctx, 0u, &dev) != 0 || bios_read_arg16(ctx, 2u, &value) != 0)
        return AMTARI_EFAULT;
    if (dev != 2u) return AMTARI_ENOSYS;
...
{
    switch (function) {
    case 0x01u: return bios_bconstat(ctx);
    case 0x02u: return bios_bconin(ctx);
    case 0x03u: return bios_bconout(ctx);
    case 0x08u: return bios_bcostat(ctx);
    case 0x0au: return (int32_t)ctx->drive_mask;
...
{
    switch (function) {
    case 0x11u: return xbios_random(ctx);
    case 0x16u: return xbios_settime(ctx);
    case 0x17u: return xbios_gettime(ctx);
    default: return AMTARI_ENOSYS;
    }
}
...
{
    switch (trap_number) {
    case 1: return AMTARI_TRAP_GEMDOS;
    case 2: return AMTARI_TRAP_GEM;
    case 13: return AMTARI_TRAP_BIOS;
    case 14: return AMTARI_TRAP_XBIOS;
    default: return AMTARI_TRAP_UNKNOWN;
    }
}
...
int32_t amtari_trap_dispatch(struct amtari_context *ctx, unsigned int trap_number, uint16_t function)
{
    ...
    case AMTARI_TRAP_GEMDOS: return amtari_gemdos_dispatch(ctx, function);
    case AMTARI_TRAP_GEM: return dispatch_gem(ctx, function);
    case AMTARI_TRAP_BIOS: return dispatch_bios(ctx, function);
    case AMTARI_TRAP_XBIOS: return dispatch_xbios(ctx, function);
