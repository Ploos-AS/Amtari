...
#define AMTARI_AES_WORD_MAX 256u
#define AMTARI_AES_ADDR_MAX 64u
...
static int32_t dispatch_vdi_trap(struct amtari_context *ctx)
{
    ...
    if (opcode == 1u) workstation_handle = ctx->vdi.next_handle;
    ...
    for (i = 0u; i < intout_count; ++i)
        if (amtari_guest_write16(ctx, intout_addr + (uint32_t)i * 2u, (uint16_t)intout[i]) != 0) return AMTARI_EFAULT;
    for (i = 0u; i < ptsout_count; ++i)
        if (amtari_guest_write16(ctx, ptsout_addr + (uint32_t)i * 2u, (uint16_t)ptsout[i]) != 0) return AMTARI_EFAULT;
    if (amtari_guest_write16(ctx, contrl + 4u, (uint16_t)(ptsout_count / 2u)) != 0 ||
        amtari_guest_write16(ctx, contrl + 8u, intout_count) != 0) return AMTARI_EFAULT;
    if (opcode == 1u) {
        if (workstation_handle == 0u) return AMTARI_EIO;
        if (amtari_guest_write16(ctx, contrl + 12u, workstation_handle) != 0) return AMTARI_EFAULT;
    }
    return 0;
}

/* AESPB is six big-endian guest pointers: control, global, intin, intout,
 * addrin and addrout. control[1..4] carry the input/output element counts. */
static int32_t dispatch_aes_trap(struct amtari_context *ctx)
{
    uint32_t pb = ctx->cpu.d[1];
    uint32_t control, global_addr, intin_addr, intout_addr, addrin_addr, addrout_addr;
    uint16_t opcode, intin_count, intout_capacity, addrin_count, addrout_capacity;
    uint16_t intout_count = 0u, addrout_count = 0u, value, i;
    int16_t intin[AMTARI_AES_WORD_MAX];
    int16_t intout[AMTARI_AES_WORD_MAX];
    uint32_t addrin[AMTARI_AES_ADDR_MAX];
    uint32_t addrout[AMTARI_AES_ADDR_MAX];
    int32_t rc;

    if (amtari_guest_read32(ctx, pb + 0u, &control) != 0 ||
        amtari_guest_read32(ctx, pb + 4u, &global_addr) != 0 ||
        amtari_guest_read32(ctx, pb + 8u, &intin_addr) != 0 ||
        amtari_guest_read32(ctx, pb + 12u, &intout_addr) != 0 ||
        amtari_guest_read32(ctx, pb + 16u, &addrin_addr) != 0 ||
        amtari_guest_read32(ctx, pb + 20u, &addrout_addr) != 0)
        return AMTARI_EFAULT;
    (void)global_addr;

    if (amtari_guest_read16(ctx, control + 0u, &opcode) != 0 ||
        amtari_guest_read16(ctx, control + 2u, &intin_count) != 0 ||
        amtari_guest_read16(ctx, control + 4u, &intout_capacity) != 0 ||
        amtari_guest_read16(ctx, control + 6u, &addrin_count) != 0 ||
        amtari_guest_read16(ctx, control + 8u, &addrout_capacity) != 0)
        return AMTARI_EFAULT;
    if (intin_count > AMTARI_AES_WORD_MAX || intout_capacity > AMTARI_AES_WORD_MAX ||
        addrin_count > AMTARI_AES_ADDR_MAX || addrout_capacity > AMTARI_AES_ADDR_MAX)
        return AMTARI_EINVAL;

    for (i = 0u; i < intin_count; ++i) {
        if (amtari_guest_read16(ctx, intin_addr + (uint32_t)i * 2u, &value) != 0) return AMTARI_EFAULT;
        intin[i] = (int16_t)value;
    }
    for (i = 0u; i < addrin_count; ++i)
        if (amtari_guest_read32(ctx, addrin_addr + (uint32_t)i * 4u, &addrin[i]) != 0) return AMTARI_EFAULT;

    rc = amtari_aes_dispatch(ctx, opcode,
                             intin_count ? intin : 0, intin_count,
                             intout, intout_capacity, &intout_count,
                             addrin_count ? addrin : 0, addrin_count,
                             addrout, addrout_capacity, &addrout_count);
    if (rc != 0) return rc;
    if (intout_count > intout_capacity || addrout_count > addrout_capacity) return AMTARI_EINVAL;

    for (i = 0u; i < intout_count; ++i)
        if (amtari_guest_write16(ctx, intout_addr + (uint32_t)i * 2u, (uint16_t)intout[i]) != 0) return AMTARI_EFAULT;
    for (i = 0u; i < addrout_count; ++i)
        if (amtari_guest_write32(ctx, addrout_addr + (uint32_t)i * 4u, addrout[i]) != 0) return AMTARI_EFAULT;
    return 0;
}
...
static int32_t dispatch_gem(struct amtari_context *ctx, uint16_t selector)
{
    ...
    if (selector == 0x00c8u) return dispatch_aes_trap(ctx);
    return AMTARI_ENOSYS;
}

static int32_t bios_bconstat(struct amtari_context *ctx)
{ uint16_t dev; int ready;if(bios_read_arg16(ctx,0u,&dev)!=0)return AMTARI_EFAULT;if(dev!=2u)return AMTARI_ENOSYS;if(ctx->console.input_ready==0)return AMTARI_EIO;ready=ctx->console.input_ready(ctx->console.opaque);if(ready<0)return AMTARI_EIO;return ready?-1:0; }
static int32_t bios_bconin(struct amtari_context *ctx)
{ uint16_t dev;int value;if(bios_read_arg16(ctx,0u,&dev)!=0)return AMTARI_EFAULT;if(dev!=2u)return AMTARI_ENOSYS;if(ctx->console.getc==0)return AMTARI_EIO;value=ctx->console.getc(ctx->console.opaque);return value<0?AMTARI_EIO:(int32_t)(value&0xff); }
static int32_t bios_bconout(struct amtari_context *ctx)
{ uint16_t dev,value;if(bios_read_arg16(ctx,0u,&dev)!=0||bios_read_arg16(ctx,2u,&value)!=0)return AMTARI_EFAULT;if(dev!=2u)return AMTARI_ENOSYS;if(ctx->console.putc==0)return AMTARI_EIO;return ctx->console.putc(ctx->console.opaque,(unsigned char)(value&0xffu))<0?AMTARI_EIO:0; }
static int32_t bios_bcostat(struct amtari_context *ctx)
{ uint16_t dev;int ready;if(bios_read_arg16(ctx,0u,&dev)!=0)return AMTARI_EFAULT;if(dev!=2u)return AMTARI_ENOSYS;if(ctx->console.output_ready==0)return AMTARI_EIO;ready=ctx->console.output_ready(ctx->console.opaque);if(ready<0)return AMTARI_EIO;return ready?-1:0; }
