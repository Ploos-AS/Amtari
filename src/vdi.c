...
{ if(ctx==0||!ctx->initialized||width==0u||height==0u||colors==0u)return AMTARI_EINVAL; ctx->vdi.width=width;ctx->vdi.height=height;ctx->vdi.colors=colors;if(ctx->vdi.next_handle==0u)ctx->vdi.next_handle=1u;return 0; }
...
{ uint16_t i;if(!ctx->vdi.line)return AMTARI_ENOSYS;if(!ptsin||n<4u||(n&1u)!=0u)return AMTARI_EINVAL;for(i=0u;i+3u<n;i=(uint16_t)(i+2u)){if(ctx->vdi.line(ctx->vdi.line_opaque,ptsin[i],ptsin[i+1u],ptsin[i+2u],ptsin[i+3u])!=0)return AMTARI_EIO;}return 0; }
