...
static int align16_checked(uint32_t value, uint32_t *aligned)
{
    if (value > UINT32_MAX - 15u) return AMTARI_ENOMEM;
    *aligned = (value + 15u) & ~15u;
    return 0;
}
...
{
    uint64_t minimum;
    if (image == 0 || info == 0 || image_size < PRG_HEADER_SIZE) return AMTARI_EINVAL;
    if (be16(image) != 0x601au) return AMTARI_ENOEXEC;
    ...
    if (minimum > image_size) return AMTARI_ENOEXEC;
    return 0;
}
                    ...
                    const struct amtari_prg_info *info, uint32_t tbase)
{
    ...
    uint32_t offset;
    if (info->absolute != 0u) return 0;
    if (pos + 4u > image_size) return AMTARI_ENOEXEC;
    offset = be32(&image[pos]); pos += 4u;
    ...
    for (;;) {
        ...
        if (pos >= image_size) return AMTARI_ENOEXEC;
        delta = image[pos++];
        if (delta == 0u) return 0;
        if (delta == 1u) offset += 254u;
        else offset += delta;
    }
}
                                ...
                                uint32_t basepage, const uint8_t *cmdline, uint32_t tail_reserve)
{
    ...
    uint32_t tbase, dbase, bbase, image_end, allocation_top, next_load;
    uint64_t image_total, allocation_total;
    size_t command_length = 0u;
    int rc;

    if (ctx == 0 || ctx->memory.data == 0) return AMTARI_EINVAL;
    ...
    if (image_total > UINT32_MAX) return AMTARI_ENOMEM;
    image_end = basepage + (uint32_t)image_total;
    if (image_end < basepage) return AMTARI_ENOMEM;

    if (tail_reserve == 0u) {
        allocation_top = image_end;
        if (align16_checked(image_end, &next_load) != 0) return AMTARI_ENOMEM;
    } else {
        if (align16_checked(image_end, &allocation_top) != 0) return AMTARI_ENOMEM;
        if (allocation_top > UINT32_MAX - tail_reserve) return AMTARI_ENOMEM;
        allocation_top += tail_reserve;
        if (align16_checked(allocation_top, &allocation_top) != 0) return AMTARI_ENOMEM;
        next_load = allocation_top;
    }

    allocation_total = (uint64_t)allocation_top - basepage;
    if (allocation_total > SIZE_MAX ||
        !amtari_guest_range_valid(ctx, basepage, (size_t)allocation_total)) return AMTARI_ENOMEM;
    ...
    if (cmdline != 0) {
        ...
        if (command_length > 124u) command_length = 124u;
    ...
    if (rc != 0) return rc;
    ctx->next_load_address = next_load;
