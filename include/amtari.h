...
#include <stddef.h>
#include <stdint.h>
#define AMTARI_VERSION "0.3.13-m3"
...
struct amtari_aes_io { amtari_aes_dispatch_fn dispatch; void *opaque; uint16_t next_application_id,current_application_id; uint8_t application_active; char application_name[9]; };
...
int amtari_aes_bind(struct amtari_context*,amtari_aes_dispatch_fn,void*); int amtari_aes_set_application_name(struct amtari_context*,const char*); int32_t amtari_aes_dispatch(struct amtari_context*,uint16_t,const int16_t*,uint16_t,int16_t*,uint16_t,uint16_t*,const uint32_t*,uint16_t,uint32_t*,uint16_t,uint16_t*);
